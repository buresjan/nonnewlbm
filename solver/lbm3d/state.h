...
//	void loadInitState(int direction, const char*subdirname);
	...
	// constructors
    #ifdef USE_CYMODEL
	    State(idx iX, idx iY, idx iZ, real iphysViscosity, real iphysDl, real iphysDt, real ilbm_nu0, real ilbm_lambda, real ilbm_a, real ilbm_n) : lbm(iX, iY, iZ, iphysViscosity, iphysDl, iphysDt, ilbm_nu0, ilbm_lambda, ilbm_a, ilbm_n)
    #elif USE_CASSON    
	    State(idx iX, idx iY, idx iZ, real iphysViscosity, real iphysDl, real iphysDt, real ilbm_k0, real ilbm_k1) : lbm(iX, iY, iZ, iphysViscosity, iphysDl, iphysDt, ilbm_k0, ilbm_k1)
    #endif
	{
		bool local_estimate = estimateMemoryDemands();
		bool global_result;
		TNLMPI::Allreduce(&local_estimate, &global_result, 1, MPI_LAND, TNLMPI::AllGroup);
		if (!local_estimate)
