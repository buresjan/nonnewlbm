...
struct LBM_STREAMING
{
	...
	CUDA_HOSTDEV static void streaming(uint8_t type, LBM_DATA &SD, KernelStruct<dreal> &KS, idx xm, idx x, idx xp, idx ym, idx y, idx yp, idx zm, idx z, idx zp)
	{
...
*/		
	...
	template < typename LBM_DATA >
	CUDA_HOSTDEV static void streamingRho(LBM_DATA &SD, KernelStruct<dreal> &KS, idx xm, idx x, idx xp, idx ym, idx y, idx yp, idx zm, idx z, idx zp)
	{
		KS.rho = SD.df(df_cur,mmm,xp+1,yp, zp) + SD.df(df_cur,mmz,xp+1,yp,z) + SD.df(df_cur,mmp,xp+1,yp,zm) + SD.df(df_cur,mzm,xp+1, y, zp) + SD.df(df_cur,mzz,xp+1, y, z) + SD.df(df_cur,mzp,xp+1, y, zm) + SD.df(df_cur,mpm,xp+1,ym,zp) + SD.df(df_cur,mpz,xp+1,ym,z) + SD.df(df_cur,mpp,xp+1,ym,zm) + SD.df(df_cur,zmm, xp,yp,zp) + SD.df(df_cur,zmz, xp,yp,z) + SD.df(df_cur,zmp, xp,yp,zm) + SD.df(df_cur,zzm, xp, y, zp) + SD.df(df_cur,zzp, xp, y, zm) + SD.df(df_cur,zzz, xp, y, z) + SD.df(df_cur,zpm, xp,ym,zp) + SD.df(df_cur,zpz, xp,ym,z) + SD.df(df_cur,zpp, xp,ym,zm) +	SD.df(df_cur,pmm,x,yp,zp) + SD.df(df_cur,pmz,x,yp,z) + SD.df(df_cur,pmp,x,yp,zm) + SD.df(df_cur,pzm,x, y,zp) + SD.df(df_cur,pzz,x, y,z) + SD.df(df_cur,pzp,x, y,zm) + SD.df(df_cur,ppm,x,ym,zp) + SD.df(df_cur,ppz,x,ym,z) + SD.df(df_cur,ppp,x,ym,zm);
	}
	
	template < typename LBM_DATA >
	CUDA_HOSTDEV static dreal streamingVx(LBM_DATA &SD, KernelStruct<dreal> &KS, idx xm, idx x, idx xp, idx ym, idx y, idx yp, idx zm, idx z, idx zp)
	{
		// (KS.f[pz] + KS.f[pm] + KS.f[pp] - KS.f[mz] - KS.f[mm] - KS.f[mp] + n1o2*KS.fx)/KS.rho;
		KS.vx =  SD.df(df_cur,pmm,xm-1,yp,zp) + SD.df(df_cur,pmz,xm-1,yp,z) + SD.df(df_cur,pmp,xm-1,yp,zm) + SD.df(df_cur,ppm,xm-1,ym,zp) + SD.df(df_cur,ppz,xm-1,ym,z) + SD.df(df_cur,ppp,xm-1,ym,zm) + SD.df(df_cur,pzm,xm-1,y,zp) + SD.df(df_cur,pzz,xm-1,y,z) + SD.df(df_cur,pzp,xm-1,y,zm) - SD.df(df_cur,mzm,x,y,zp) - SD.df(df_cur,mzz,x,y,z) - SD.df(df_cur,mzp,x,y,zm) - SD.df(df_cur,mmm,x,yp,zp) - SD.df(df_cur,mmz,x,yp,z) - SD.df(df_cur,mmp,x,yp,zm) - SD.df(df_cur,mpm,x,ym,zp) - SD.df(df_cur,mpz,x,ym,z) - SD.df(df_cur,mpp,x,ym,zm);
	}


	template < typename LBM_DATA >
	CUDA_HOSTDEV static dreal streamingVy(LBM_DATA &SD, KernelStruct<dreal> &KS, idx xm, idx x, idx xp, idx ym, idx y, idx yp, idx zm, idx z, idx zp)
	{
		// (KS.f[pz] + KS.f[pm] + KS.f[pp] - KS.f[mz] - KS.f[mm] - KS.f[mp] + n1o2*KS.fx)/KS.rho;
		KS.vy = SD.df(df_cur,mpm,x,ym,zp) + SD.df(df_cur,mpz,x,ym,z) + SD.df(df_cur,mpp,x,ym,zm) + SD.df(df_cur,zpm,xm,ym,zp) + SD.df(df_cur,zpz,xm,ym,z) + SD.df(df_cur,zpp,xm,ym,zm) + SD.df(df_cur,ppm,xm-1,ym,zp) + SD.df(df_cur,ppz,xm-1,ym,z) + SD.df(df_cur,ppp,xm-1,ym,zm) - SD.df(df_cur,zmm,xm,yp,zp) - SD.df(df_cur,zmz,xm,yp,z) - SD.df(df_cur,zmp,xm,yp,zm) - SD.df(df_cur,pmm,xm-1,yp,zp) - SD.df(df_cur,pmz,xm-1,yp,z) - SD.df(df_cur,pmp,xm-1,yp,zm) - SD.df(df_cur,mmm,x,yp,zp) - SD.df(df_cur,mmz,x,yp,z) - SD.df(df_cur,mmp,x,yp,zm);
	}
	
	template < typename LBM_DATA >
	CUDA_HOSTDEV static dreal streamingVz(LBM_DATA &SD, KernelStruct<dreal> &KS, idx xm, idx x, idx xp, idx ym, idx y, idx yp, idx zm, idx z, idx zp)
	{
		// (KS.f[pz] + KS.f[pm] + KS.f[pp] - KS.f[mz] - KS.f[mm] - KS.f[mp] + n1o2*KS.fx)/KS.rho;
		KS.vz = SD.df(df_cur,mmp,x,yp,zm) + SD.df(df_cur,pmp,xm-1,yp,zm) + SD.df(df_cur,zmp,xm,yp,zm) + SD.df(df_cur,pzp,xm-1,y,zm) + SD.df(df_cur,zzp,xm,y,zm) + SD.df(df_cur,mzp,x,y,zm) + SD.df(df_cur,ppp,xm-1,ym,zm) + SD.df(df_cur,zpp,xm,ym,zm) + SD.df(df_cur,mpp, x,ym,zm) - (SD.df(df_cur,mmm,x,yp,zp) + SD.df(df_cur,pmm,xm-1,yp,zp) + SD.df(df_cur,zmm,xm,yp,zp) + SD.df(df_cur,pzm,xm-1,y,zp) + SD.df(df_cur,zzm,xm,y,zp) + SD.df(df_cur,mzm,x,y,zp) + SD.df(df_cur,ppm,xm-1,ym,zp) + SD.df(df_cur,zpm,xm,ym,zp) + SD.df(df_cur,mpm, x,ym,zp));
    }
};
