...
//		return true;
	...
	TNLMPI::Bcast(&result, 1, 0, TNLMPI::AllGroup);
	return result;
}
...
//			sprintf(fname,"%s/%s_%06d_t%f", dir, probe1Dvec[i].name, probe1Dvec[i].cycle, lbm.physTime());
			sprintf(fname,"%s/%s_rank%03d_%06d", dir, probe1Dvec[i].name, lbm.rank, probe1Dvec[i].cycle);
...
//			probeLine(probe1Dvec[i].from[0],probe1Dvec[i].from[1],probe1Dvec[i].from[2],probe1Dvec[i].to[0],probe1Dvec[i].to[1],probe1Dvec[i].to[2],fname);
	...
	for (int i=0;i<probe1Dlinevec.size(); i++)
	{
		...
		char fname[FILENAME_CHARS];
//		sprintf(fname,"%s/%s_%06d_t%f", dir, probe1Dvec[i].name, probe1Dvec[i].cycle, lbm.physTime());
		sprintf(fname,"%s/%s_rank%03d_%06d", dir, probe1Dlinevec[i].name, lbm.rank, probe1Dlinevec[i].cycle);
		// create parent directories
		create_file(fname);
...
void State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::writeVTKs_3Dcut()
{
	...
	{
		char fname[FILENAME_CHARS];
		sprintf(fname,"%s/%s_rank%03d_%d.vtk", dir, probe3Dvec[i].name, lbm.rank, probe3Dvec[i].cycle);
		// create parent directories
		create_file(fname);
		writeVTK_3Dcut(
			fname, 
...
void State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::writeVTKs_2D()
{
	...
	{
		char fname[FILENAME_CHARS];
		sprintf(fname,"%s/%s_rank%03d_%d.vtk", dir, probe2Dvec[i].name, lbm.rank, probe2Dvec[i].cycle);
		// create parent directories
		create_file(fname);
...
template< typename LBM_TYPE, typename MACRO, typename CPU_MACRO, typename LBM_DATA, typename LBM_BC>
bool State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::projectPNG_X(const char * filename, idx x0, bool rotate, bool mirror, bool flip)
{
...
template< typename LBM_TYPE, typename MACRO, typename CPU_MACRO, typename LBM_DATA, typename LBM_BC>
bool State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::projectPNG_Y(const char * filename, idx y0, bool rotate, bool mirror, bool flip)
{
...
template< typename LBM_TYPE, typename MACRO, typename CPU_MACRO, typename LBM_DATA, typename LBM_BC>
bool State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::projectPNG_Z(const char * filename, idx z0, bool rotate, bool mirror, bool flip)
{
...
template< typename LBM_TYPE, typename MACRO, typename CPU_MACRO, typename LBM_DATA, typename LBM_BC>
void State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::move(const char* srcdir, const char* dstdir, const char* srcfilename, const char* dstfilename)
{
...
//template< typename LBM_TYPE, typename MACRO, typename CPU_MACRO, typename LBM_DATA, typename LBM_BC>
//template< typename... ARGS >
//int State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::saveLoadTextData(int direction, const char*dirname, const char*filename, const char*fmt, ARGS&... args)
//{
...
template< typename LBM_TYPE, typename MACRO, typename CPU_MACRO, typename LBM_DATA, typename LBM_BC>
template< typename... ARGS >
int State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::saveLoadTextData(int direction, const char*dirname, const char*filename, ARGS&... args)
{
	// check if main dir exists
	mkdir_p(dirname, 0777);
	char fname[FILENAME_CHARS];
...
int State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::saveloadBinaryData(int direction, const char*dirname, const char*filename, VARTYPE*data, idx length)
{
	// check if main dir exists
	mkdir_p(dirname, 0777);
	char fname[FILENAME_CHARS];
...
bool State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::wallTimeReached()
{
	...
	bool result;
	TNLMPI::Allreduce(&local_result, &result, 1, MPI_LOR, TNLMPI::AllGroup);
	return result;
}

template< typename LBM_TYPE, typename MACRO, typename CPU_MACRO, typename LBM_DATA, typename LBM_BC>
double State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::getWallTime(bool collective)
{
	double walltime = 0;
	...
	if (collective)
	{
		...
		TNLMPI::Bcast(&walltime, 1, 0, TNLMPI::AllGroup);
	}
	return walltime;
}
...
template< typename LBM_TYPE, typename MACRO, typename CPU_MACRO, typename LBM_DATA, typename LBM_BC>
bool State<LBM_TYPE, MACRO, CPU_MACRO, LBM_DATA, LBM_BC>::estimateMemoryDemands()
{
	long long memDFs = lbm.local_X*lbm.local_Y*lbm.local_Z*27*sizeof(dreal);
	long long memMacro = lbm.local_X*lbm.local_Y*lbm.local_Z*sizeof(dreal)*MACRO::N;
